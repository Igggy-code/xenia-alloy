...
- (NSDragOperation)draggingEntered:(id<NSDraggingInfo>)sender {
  if (!xenia_window_ || !xenia_window_->CanAcceptFileDrop()) {
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  ...
  // Like the Windows frontend, open only the first dropped title.
  NSURL* url = urls.firstObject;
  const char* filename = url.fileSystemRepresentation;
  if (!filename) {
    return NO;
  }
  xenia_window_->HandleFileDrop(std::filesystem::path(filename));
  return YES;
}
...
bool MacWindow::OpenImpl() {
  ...
  NSTimer* timer =
      [NSTimer scheduledTimerWithTimeInterval:1.0 / 60.0  // 60 FPS
                                      repeats:YES
                                        block:^(NSTimer* timer) {
                                          if (weakSelf && weakSelf->content_view_) {
                                            [weakSelf->content_view_ setNeedsDisplay:YES];
                                            // Also directly trigger OnPaint to ensure ImGui gets
                                            // updated.
                                            weakSelf->OnPaint();
                                          }
                                        }];
...
void MacWindow::ApplyNewMainMenu(MenuItem* old_main_menu) {
  ...
  // AppKit reserves the first menu for the application. Keep it separate
  // from the cross-platform File menu so File remains visible as its own menu.
  NSString* app_name = @"Xenia Canary";
  NSMenuItem* application_item =
      [[NSMenuItem alloc] initWithTitle:app_name action:nil keyEquivalent:@""];
  NSMenu* application_menu = [[NSMenu alloc] initWithTitle:app_name];
  [application_menu addItemWithTitle:[@"About " stringByAppendingString:app_name]
                             action:@selector(orderFrontStandardAboutPanel:)
                      keyEquivalent:@""];
  [application_menu addItem:[NSMenuItem separatorItem]];
  [application_menu addItemWithTitle:[@"Hide " stringByAppendingString:app_name]
                             action:@selector(hide:)
                      keyEquivalent:@"h"];
  NSMenuItem* hide_others =
      [application_menu addItemWithTitle:@"Hide Others"
                                  action:@selector(hideOtherApplications:)
                           keyEquivalent:@"h"];
  [hide_others setKeyEquivalentModifierMask:
                   NSEventModifierFlagCommand | NSEventModifierFlagOption];
  [application_menu addItemWithTitle:@"Show All"
                             action:@selector(unhideAllApplications:)
                      keyEquivalent:@""];
  [application_menu addItem:[NSMenuItem separatorItem]];
  [application_menu addItemWithTitle:[@"Quit " stringByAppendingString:app_name]
...
bool MacWindow::CanAcceptFileDrop() const {
  return window_ && !is_closing_;
}
...
void MacWindow::HandleFileDrop(const std::filesystem::path& path) {
  ...
  XELOGI("macOS file drop: {}", path.string());
  FileDropEvent event(this, path);
  WindowDestructionReceiver destruction_receiver(this);
  OnFileDrop(event, destruction_receiver);
  // The callback may have closed or destroyed this window.
}
...
void MacWindow::HandleSizeUpdate() {
  ...
  WindowDestructionReceiver destruction_receiver(this);
  OnActualSizeUpdate(logical_width, logical_height,
...
void MacWindow::HandleKeyEvent(void* event, bool is_down) {
  ...
  switch (key_code) {
    case 48:
      virtual_key = VirtualKey::kTab;
      break;  // Tab
    case 36:
      virtual_key = VirtualKey::kReturn;
      break;  // Return
    case 51:
      virtual_key = VirtualKey::kBack;
      break;  // Backspace
    case 53:
      virtual_key = VirtualKey::kEscape;
      break;  // Escape
    case 49:
      virtual_key = VirtualKey::kSpace;
      break;  // Space
    case 114:
      virtual_key = VirtualKey::kInsert;
      break;  // Insert
    case 115:
      virtual_key = VirtualKey::kHome;
      break;  // Home
    case 119:
      virtual_key = VirtualKey::kEnd;
      break;  // End
    case 116:
      virtual_key = VirtualKey::kPrior;
      break;  // Page Up
    case 121:
      virtual_key = VirtualKey::kNext;
      break;  // Page Down
    case 117:
      virtual_key = VirtualKey::kDelete;
      break;  // Delete
    case 123:
      virtual_key = VirtualKey::kLeft;
      break;  // Left
    case 124:
      virtual_key = VirtualKey::kRight;
      break;  // Right
    case 125:
      virtual_key = VirtualKey::kDown;
      break;  // Down
    case 126:
      virtual_key = VirtualKey::kUp;
      break;  // Up
    case 56:
      virtual_key = VirtualKey::kShift;
      break;  // LShift
    case 60:
      virtual_key = VirtualKey::kShift;
      break;  // RShift
    case 59:
      virtual_key = VirtualKey::kControl;
      break;  // LCtrl
    case 62:
      virtual_key = VirtualKey::kControl;
      break;  // RCtrl
    case 58:
      virtual_key = VirtualKey::kMenu;
      break;  // LAlt
    case 61:
      virtual_key = VirtualKey::kMenu;
      break;  // RAlt
    case 55:
      virtual_key = VirtualKey::kLWin;
      break;  // LCmd
    case 54:
      virtual_key = VirtualKey::kLWin;
      break;  // RCmd
    case 18:
      virtual_key = VirtualKey::k1;
      break;  // 1
    case 19:
      virtual_key = VirtualKey::k2;
      break;  // 2
    case 20:
      virtual_key = VirtualKey::k3;
      break;  // 3
    case 21:
      virtual_key = VirtualKey::k4;
      break;  // 4
    case 23:
      virtual_key = VirtualKey::k5;
      break;  // 5
    case 22:
      virtual_key = VirtualKey::k6;
      break;  // 6
    case 26:
      virtual_key = VirtualKey::k7;
      break;  // 7
    case 28:
      virtual_key = VirtualKey::k8;
      break;  // 8
    case 25:
      virtual_key = VirtualKey::k9;
      break;  // 9
    case 29:
      virtual_key = VirtualKey::k0;
      break;  // 0
    case 122:
      virtual_key = VirtualKey::kF1;
      break;  // F1
    case 120:
      virtual_key = VirtualKey::kF2;
      break;  // F2
    case 99:
      virtual_key = VirtualKey::kF3;
      break;  // F3
    case 118:
      virtual_key = VirtualKey::kF4;
      break;  // F4
    case 96:
      virtual_key = VirtualKey::kF5;
      break;  // F5
    case 97:
      virtual_key = VirtualKey::kF6;
      break;  // F6
    case 98:
      virtual_key = VirtualKey::kF7;
      break;  // F7
    case 100:
      virtual_key = VirtualKey::kF8;
      break;  // F8
    case 101:
      virtual_key = VirtualKey::kF9;
      break;  // F9
    case 109:
      virtual_key = VirtualKey::kF10;
      break;  // F10
    case 103:
      virtual_key = VirtualKey::kF11;
      break;  // F11
    case 111:
      virtual_key = VirtualKey::kF12;
      break;  // F12
    case 50:
      virtual_key = VirtualKey::kOem3;
      break;  // ` (backtick)
    // Letter keys
    case 0:
      virtual_key = VirtualKey::kA;
      break;  // A
    case 11:
      virtual_key = VirtualKey::kB;
      break;  // B
    case 8:
      virtual_key = VirtualKey::kC;
      break;  // C
    case 2:
      virtual_key = VirtualKey::kD;
      break;  // D
    case 14:
      virtual_key = VirtualKey::kE;
      break;  // E
    case 3:
      virtual_key = VirtualKey::kF;
      break;  // F
    case 5:
      virtual_key = VirtualKey::kG;
      break;  // G
    case 4:
      virtual_key = VirtualKey::kH;
      break;  // H
    case 34:
      virtual_key = VirtualKey::kI;
      break;  // I
    case 38:
      virtual_key = VirtualKey::kJ;
      break;  // J
    case 40:
      virtual_key = VirtualKey::kK;
      break;  // K
    case 37:
      virtual_key = VirtualKey::kL;
      break;  // L
    case 46:
      virtual_key = VirtualKey::kM;
      break;  // M
    case 45:
      virtual_key = VirtualKey::kN;
      break;  // N
    case 31:
      virtual_key = VirtualKey::kO;
      break;  // O
    case 35:
      virtual_key = VirtualKey::kP;
      break;  // P
    case 12:
      virtual_key = VirtualKey::kQ;
      break;  // Q
    case 15:
      virtual_key = VirtualKey::kR;
      break;  // R
    case 1:
      virtual_key = VirtualKey::kS;
      break;  // S
    case 17:
      virtual_key = VirtualKey::kT;
      break;  // T
    case 32:
      virtual_key = VirtualKey::kU;
      break;  // U
    case 9:
      virtual_key = VirtualKey::kV;
      break;  // V
    case 13:
      virtual_key = VirtualKey::kW;
      break;  // W
    case 7:
      virtual_key = VirtualKey::kX;
      break;  // X
    case 16:
      virtual_key = VirtualKey::kY;
      break;  // Y
    case 6:
      virtual_key = VirtualKey::kZ;
      break;  // Z
    default:
  ...
  KeyEvent e(this, virtual_key, 1, !is_down, shift_pressed, ctrl_pressed, alt_pressed,
             super_pressed);
  WindowDestructionReceiver destruction_receiver(this);
...
NSMenuItem* MacWindow::CreateNSMenuItemFromMenuItem(MenuItem* menu_item) {
  ...
  switch (menu_item->type()) {
    ...
    case MenuItem::Type::kString: {
      ...
      if (menu_item->hotkey() == "Ctrl+O") {
        key_equivalent = @"o";
        modifier_flags = NSEventModifierFlagCommand;
      } else if (menu_item->hotkey() == "Cmd+G") {
        key_equivalent = @"g";
        modifier_flags = NSEventModifierFlagCommand;
