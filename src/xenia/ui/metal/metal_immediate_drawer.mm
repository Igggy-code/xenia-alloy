...
static MTL::Function* CreateFunction(MTL::Device* device, const void* data, size_t size,
  ...
  MTL::Library* lib = util::CreateLibrary(device, data, size, &error);
  if (!lib) {
...
void MetalImmediateDrawer::BeginDrawBatch(const ImmediateDrawBatch& batch) {
  ...
  if (batch.indices && batch.index_count > 0) {
    ...
    id<MTLBuffer> index_buffer = [mtl_device newBufferWithBytes:batch.indices
                                                        ...
                                                        options:MTLResourceStorageModeShared];
    // newBuffer returns an owned reference; preserve it in void* storage.
    // The buffer will be released in EndDrawBatch().
...
void MetalImmediateDrawer::EndDrawBatch() {
  assert_true(batch_open_);
  batch_open_ = false;
  // Release the index buffer that was retained with __bridge_retained in BeginDrawBatch.
  if (current_index_buffer_) {
