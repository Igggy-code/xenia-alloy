...
bool MetalPresenter::EnsureApplyGammaPipelines() {
  ...
  auto create_pipeline = [&](const uint8_t* data, size_t size,
    ...
    NSError* error = nil;
    id<MTLLibrary> library = (__bridge id<MTLLibrary>)util::CreateLibrary(
        device_, data, size, reinterpret_cast<NS::Error**>(&error));
    if (!library) {
...
id<MTLFunction> CreateGuestOutputFunction(id<MTLDevice> device, const uint8_t* data, size_t size,
  ...
  NSError* error = nil;
  id<MTLLibrary> library = (__bridge id<MTLLibrary>)util::CreateLibrary(
      (__bridge MTL::Device*)device, data, size, reinterpret_cast<NS::Error**>(&error));
  if (!library) {
