...
#else
    // Hosts with an allocation granularity above 4 KB (16 KB pages on Apple
    // Silicon) can't map the 0xE0000000 physical view at the +4 KB file
    // offset, so the offset is applied here, matching the JIT and the
    // vE0000000 heap's host_address_offset.
