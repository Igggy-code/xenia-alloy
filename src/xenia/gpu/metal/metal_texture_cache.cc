...
void MetalTextureCache::NoteGpuWrittenRange(MTL::CommandBuffer* command_buffer,
                                            uint32_t start, uint32_t length) {
  if (!command_buffer || !length) {
    return;
  }
  if (gpu_written_ranges_command_buffer_ != command_buffer) {
    gpu_written_ranges_command_buffer_ = command_buffer;
    gpu_written_ranges_.clear();
  }
  gpu_written_ranges_.emplace_back(start, start + length);
}

bool MetalTextureCache::IsRangeGpuWrittenInCommandBuffer(
    MTL::CommandBuffer* command_buffer, uint32_t start,
  ...
  for (const auto& range : gpu_written_ranges_) {
...
bool MetalTextureCache::TryGpuLoadTexture(Texture& texture, bool load_base,
  ...
  // If the texture data was written by the GPU (resolve) in the current,
  // not yet committed, command buffer, the load must be encoded after that
  // write in the same command buffer - a separately committed command buffer
  // would execute first and load stale data from the previous frame.
  if (use_blit_upload && command_processor_ && current_command_buffer &&
      !use_current_command_buffer &&
      ((load_base &&
        IsRangeGpuWrittenInCommandBuffer(current_command_buffer,
                                         base_guest_address,
    ...
    use_current_command_buffer = true;
  }
  if (use_upload_batch && texture_resolution_scaled) {
    ...
    if (needs_base_scaled_range &&
        !IsScaledResolveRangeResident(base_guest_address,
                                      texture.GetGuestBaseSize(),
                                      4 /* current XeSL byte-buffer alignment */)) {
...
MTL::Texture* MetalTextureCache::CreateNullTexture2D() {
  ...
  if (texture) {
    ...
    // Transparent black like a null descriptor in Direct3D 12 - an opaque
    // black null texture turned blended draws with a missing texture into
    // solid black polygons.
...
MTL::Texture* MetalTextureCache::GetTextureForBinding(
  ...
  if (!AreDimensionsCompatible(dimension, binding->key.dimension)) {
    static uint32_t dimension_mismatch_log_count = 0;
    if (dimension_mismatch_log_count < 16) {
      ++dimension_mismatch_log_count;
      XELOGW(
          "GetTextureForBinding: fetch {} dimension mismatch (shader {}, "
          "texture {}, format {}, {}x{}x{}) - using a null texture",
          fetch_constant, uint32_t(dimension),
          uint32_t(binding->key.dimension), uint32_t(binding->key.format),
          binding->key.GetWidth(), binding->key.GetHeight(),
          binding->key.GetDepthOrArraySize());
    }
    return get_null_texture_for_dimension();
  }
...
MTL::SamplerState* MetalTextureCache::GetOrCreateSampler(
  ...
  // The cache key must be the guest parameters as requested - they are
  // modified below by debug overrides (such as metal_force_linear_filter),
  // and caching under the modified value made every lookup miss, creating a
  // new MTLSamplerState on every draw.
  const auto cache_key = parameters.value;
  auto it = sampler_cache_.find(cache_key);
  ...
  {
    static uint32_t sampler_log_count = 0;
    if (sampler_log_count < 32) {
      ++sampler_log_count;
      XELOGI(
          "MetalSampler #{}: mag_linear={} min_linear={} mip_linear={} "
          "aniso={} mip_base_map={} mip_min_level={} clamp={}/{}/{}",
          sampler_log_count, uint32_t(parameters.mag_linear),
          uint32_t(parameters.min_linear), uint32_t(parameters.mip_linear),
          uint32_t(parameters.aniso_filter), uint32_t(parameters.mip_base_map),
          uint32_t(parameters.mip_min_level), uint32_t(parameters.clamp_x),
  ...
  sampler_cache_.emplace(cache_key, sampler_state);
