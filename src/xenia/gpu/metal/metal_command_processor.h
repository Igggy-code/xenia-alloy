...
#undef OVERRIDING_BASE_CMDPROCESSOR
 ...
 private:
  ...
  // Additional constant depth bias (in minimum resolvable depth units) for the
  // current draw, see IssueDraw.
  float depth_bias_extra_constant_ = 0.0f;
  ...
  // Depth-only fragment shader converting depth to float24 (for null-PS draws
  // when depth_float24_convert_in_pixel_shader is enabled).
  bool EnsureDepthOnlyFloat24PixelShader();
  bool IsFloat24DepthConversionActive(const RegisterFile& regs) const;
  ...
  // translated shaders only access the slots of the textures and samplers
  // they actually bind (slot 0 is shared memory, then one slot per binding,
  // bounded by the guest's 32 fetch constants and their variants). Allocating
  // the full declared ranges for every draw made each draw-ring page ~20 MB,
  // quickly exhausting the in-flight pool (1 GB) and stalling the GPU thread.
  static constexpr size_t kResourceHeapSlotsPerTable = 128 + 2;
  static constexpr size_t kSamplerHeapSlotsPerTable = 64 + 2;
 ...
 private:
  ...
  MTL::Library* depth_only_float24_pixel_library_ = nullptr;
  std::string depth_only_float24_pixel_function_name_;
  ...
  struct GpuWriteStreak {
    uint32_t last_swap = UINT32_MAX;
    uint32_t frames = 0;
  };
  std::unordered_map<uint32_t, GpuWriteStreak> gpu_write_streaks_;
  uint64_t guest_sync_skipped_writes_ = 0;
  // Incremented on every swap - the frame clock for gpu_write_streaks_.
