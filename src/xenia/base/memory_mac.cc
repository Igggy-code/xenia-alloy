...
static std::once_flag g_cleanup_handlers_once;
...
static void InstallCleanupHandlers() {
  std::call_once(g_cleanup_handlers_once, [] {
...
bool Protect(void* base_address, size_t length, PageAccess access,
  ...
  const uintptr_t aligned_end = xe::align(start + length, system_page_size);
  if (aligned_end < start || aligned_end < aligned_start) {
    return false;
  }
  uint32_t prot = ToPosixProtectFlags(access);
...
bool QueryProtect(void* base_address, size_t& length, PageAccess& access_out) {
  ...
  if (kr != KERN_SUCCESS || address > reinterpret_cast<uintptr_t>(base_address)) {
    // mach_vm_region returns the next mapping when the queried address is a gap.
    return false;
  }
...
void* MapFileView(FileMappingHandle handle, void* base_address, size_t length,
  ...
  mach_vm_address_t reservation = reinterpret_cast<uintptr_t>(base_address);
  const size_t reserved_length = xe::align(length, page_size());
  if (base_address) {
    if (!length || (reservation & (page_size() - 1)) ||
        reserved_length < length) {
      return nullptr;
    }
    // Darwin has no MAP_FIXED_NOREPLACE. First atomically claim the requested
    // range without replacing anything, then map over our own reservation.
    if (mach_vm_allocate(mach_task_self(), &reservation, reserved_length,
  ...
  if (result == MAP_FAILED) {
    if (base_address) {
      mach_vm_deallocate(mach_task_self(), reservation, reserved_length);
    }
    return nullptr;
  }
  std::lock_guard guard(g_mapped_file_ranges_mutex);
