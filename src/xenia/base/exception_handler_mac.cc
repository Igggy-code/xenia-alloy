...
static void ExceptionHandlerCallback(int signal_number, siginfo_t* signal_info,
  ...
  //
  // Only the header up to and including the uc_mcontext pointer is copied:
  // with _XOPEN_SOURCE, macOS' ucontext_t also embeds __mcontext_data, which
  // the kernel's signal frame doesn't have in that place. Copying the whole
  // ucontext_t (and especially copying it back) clobbered the real mcontext
  // that uc_mcontext points to, so a modified PC (MMIO emulation) was lost
  // and the faulting instruction was re-executed forever.
  alignas(16) ucontext_t ucontext_storage;
  std::memcpy(&ucontext_storage, signal_context,
              offsetof(ucontext_t, uc_mcontext) +
                  sizeof(ucontext_storage.uc_mcontext));
  ...
  for (size_t i = 0; i < xe::countof(handlers_) && handlers_[i].first; ++i) {
    if (handlers_[i].first(&ex, handlers_[i].second)) {
      ...
      // uc_mcontext points to the real signal frame's mcontext, which has been
      // modified in place above - nothing needs to be copied back.
#elif XE_ARCH_AMD64
      ...
      uint16_t modified_xmm = ex.modified_xmm_registers();
      while (xe::bit_scan_forward(modified_xmm, &modified_register_index)) {
        modified_xmm &= ~(UINT16_C(1) << modified_register_index);
  ...
  // Unhandled synchronous faults must reach the previous handler (or terminate
  // normally); returning unchanged would retry the fault forever.
  const struct sigaction* previous = signal_number == SIGILL
  ...
  if (previous->sa_handler != SIG_DFL && previous->sa_handler != SIG_IGN) {
    if (previous->sa_flags & SA_SIGINFO) {
      previous->sa_sigaction(signal_number, signal_info, signal_context);
    } else {
  ...
  sigaction(signal_number, &default_handler, nullptr);
  raise(signal_number);
}
...
void ExceptionHandler::Install(Handler fn, void* data) {
  if (!signal_handlers_installed_) {
    ...
    // SA_NODEFER: MMIO callbacks run inside this handler and may touch guest
    // memory that is write-watched (e.g. XMA context Clear/Lock). With the
    // signal blocked, the nested fault is never delivered on macOS and the
    // thread spins on the faulting instruction forever. Allow nesting, like
    // vectored exception handlers on Windows.
