...
// User callbacks run only at alertable waits, outside any signal handler or
// condition lock. This also preserves callbacks queued before a wait begins.
thread_local bool alertable_state_ = false;
static bool DispatchCurrentThreadCallbacks();

class AlertableScope {
 public:
  explicit AlertableScope(bool alertable) : previous_(alertable_state_) {
    alertable_state_ = alertable;
  }
  ~AlertableScope() { alertable_state_ = previous_; }
 private:
  bool previous_;
};

SleepResult AlertableSleep(std::chrono::microseconds duration) {
  AlertableScope scope(true);
  const auto deadline = std::chrono::steady_clock::now() + duration;
  while (true) {
    if (DispatchCurrentThreadCallbacks()) {
    ...
    NanoSleep(std::min(remaining, std::chrono::nanoseconds(1000000)).count());
  }
}
 ...
 public:
  ...
  WaitResult Wait(std::chrono::milliseconds timeout) {
    ...
    while (true) {
      if (alertable_state_) {
        lock.unlock();
        const bool called = DispatchCurrentThreadCallbacks();
        lock.lock();
        if (called) {
      ...
      if (alertable_state_) {
        cond_.wait_until(lock, std::min(deadline, now + std::chrono::milliseconds(1)));
  ...
  static std::pair<WaitResult, size_t> WaitMultiple(
    ...
    while (true) {
      if (alertable_state_ && DispatchCurrentThreadCallbacks()) {
...
class PosixCondition<Event> : public PosixConditionBase {
 public:
  ...
  EventInfo Query() {
    ...
    return {manual_reset_ ? 0u : 1u, signal_ ? 1u : 0u};
  }
...
struct ThreadStartData {
  ...
  std::shared_ptr<PosixCondition<Thread>> condition;
};
...
class PosixCondition<Thread> final : public PosixConditionBase {
 ...
 public:
  PosixCondition()
      : thread_(0),
        ...
        suspend_count_(0),
        joined_(false) {
    const kern_return_t result = semaphore_create(
        mach_task_self(), &suspend_sem_, SYNC_POLICY_FIFO, 0);
    assert_zero(result);
#if XE_PLATFORM_ANDROID
  ...
  ~PosixCondition() override {
    // The running pthread keeps its condition alive even after its public
    // handle is closed. Reap an unobserved thread when that last reference
    // goes away; a waiter may already have joined it in post_execution().
    if (owns_thread_ && !joined_) {
  ...
  bool CallUserCallbacks() {
    std::deque<std::function<void()>> callbacks;
    {
      std::lock_guard lock(callback_mutex_);
      callbacks.swap(user_callbacks_);
    }
    const bool called = !callbacks.empty();
  ...
  void Terminate(int exit_code) {
    bool is_current_thread = pthread_self() == thread_;
    {
      ...
      if (state_ == State::kFinished) {
        if (is_current_thread) {
          // A concurrent terminator may already have marked us finished.
          // Leave under our own power rather than spinning for cancellation.
          lock.unlock();
  ...
  // Signal handlers must not lock a mutex: the interrupted thread may own it.
  // Mach semaphores use kernel traps and preserve the upstream suspend counter
  // semantics without relying on unsupported unnamed POSIX semaphores.
  void WaitSuspended() {
    while (suspend_count_.load(std::memory_order_acquire) != 0) {
      kern_return_t result;
      do {
        result = semaphore_wait(suspend_sem_);
 ...
 private:
  ...
  std::atomic<uint32_t> suspend_count_;  // Also read by signal handler.
...
// Unlike other handles, a thread's state is also owned by the running native
// thread. Closing the caller's handle must not invalidate Thread::Exit or the
// completion path in ThreadStartRoutine.
template <>
class PosixConditionHandle<Thread> : public Thread, public PosixWaitHandle {
 public:
  PosixConditionHandle()
      : owned_handle_(std::make_shared<PosixCondition<Thread>>()),
        handle_(*owned_handle_) {}
  explicit PosixConditionHandle(pthread_t thread)
      : owned_handle_(std::make_shared<PosixCondition<Thread>>(thread)),
        handle_(*owned_handle_) {}
  explicit PosixConditionHandle(
      std::shared_ptr<PosixCondition<Thread>> condition)
      : owned_handle_(std::move(condition)), handle_(*owned_handle_) {}

  PosixCondition<Thread>& condition() override { return handle_; }
  void* native_handle() const override { return handle_.native_handle(); }

 protected:
  std::shared_ptr<PosixCondition<Thread>> owned_handle_;
  PosixCondition<Thread>& handle_;
  friend PosixCondition<Thread>;
};
...
WaitResult Wait(WaitHandle* wait_handle, bool is_alertable,
  ...
  AlertableScope scope(is_alertable);
...
class PosixThread final : public PosixConditionHandle<Thread> {
 public:
  ...
  explicit PosixThread(std::shared_ptr<PosixCondition<Thread>> condition)
      : PosixConditionHandle(std::move(condition)) {}
  ~PosixThread() override = default;

  bool Initialize(CreationParameters params,
                  std::function<void()> start_routine) {
    auto start_data =
        new ThreadStartData({std::move(start_routine), false, owned_handle_});
...
void* PosixCondition<Thread>::ThreadStartRoutine(void* parameter) {
  ...
  assert_not_null(start_data->condition.get());

  struct RunningThread {
    explicit RunningThread(ThreadStartData& data)
        : thread(std::move(data.condition)),
          routine(std::move(data.start_routine)) {}
    PosixThread thread;
    std::function<void()> routine;
  };
  auto running = new RunningThread(*start_data);
  auto create_suspended = start_data->create_suspended;
  delete start_data;

  auto& thread = running->thread;
  current_thread_ = &thread;
  // Darwin pthread_exit does not unwind C++ stack objects. A POSIX cleanup
  // handler releases the running thread's state and callback on every exit,
  // even after the public Thread handle has already been closed.
  pthread_cleanup_push(+[](void* value) {
    current_thread_ = nullptr;
    delete static_cast<RunningThread*>(value);
  }, running);
  {
